...
mkdir -p output-biobert/predictions/dis-blocklists-v12
...
#NAME="dis-contexts-w100-0[0-9][0-9].tsv"
#NAME="dis-contexts-w100-1[0-9][0-9].tsv"
NAME="dis-contexts-w100-2[0-9][0-9].tsv"
    ...
    if [[ "${dataset##*./}" =~ ${NAME} ]]; then
        ...
        for model in $MODELS; do
            if [[ "$model" =~ "2595163" ]]; then
                ...
                while true; do
                #change to base for base model
                    jobs=$(ls output-biobert/predictions/dis-blocklists-v12 | wc -l)
                    if [ $jobs -lt $MAX_JOBS ]; then break; fi
                        echo "Too many jobs ($jobs), sleeping ..."
                        sleep 60
                done
            ...
            echo "Submitting job with params $model $dataset $max_seq_len $batch_size"
                job_id=$(
                sbatch slurm/slurm-run-predict-dis.sh \
                    $config_dir \
                    $current_data_dir \
                    $max_seq_len \
                    $batch_size \
            ...
            sleep 5
            if [[ "$model" =~ "11515370" ]]; then touch output-biobert/predictions/dis-blocklists-v12/$job_id; fi
            sleep 5
        done
    fi
done
