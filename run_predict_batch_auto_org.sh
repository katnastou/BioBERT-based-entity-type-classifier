...
#NAME="org-contexts-w100-0[0-9][0-9].tsv"
#NAME="org-contexts-w100-1[0-9][0-9].tsv"
#NAME="org-contexts-w100-2[0-9][0-9].tsv"
NAME="org-contexts-w100-092.tsv"
    ...
    if [[ "${dataset##*./}" =~ ${NAME} ]]; then
        ...
        for model in $MODELS; do
            ...
            echo "Submitting job with params $model $dataset $max_seq_len $batch_size"
                job_id=$(
                sbatch slurm/slurm-run-predict-org.sh \
                    $config_dir \
                    $current_data_dir \
                    $max_seq_len \
                    $batch_size \
            ...
            sleep 5
            if [[ "$model" =~ "11515370" ]]; then touch output-biobert/predictions/org-blocklists-v12/$job_id; fi
            sleep 5
        done
    fi
done
