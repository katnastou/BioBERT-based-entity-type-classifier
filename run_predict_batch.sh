...
mkdir -p output-biobert/predictions/base_new_week8_dis
...
data_dir="/scratch/project_2001426/stringdata/week_8/split-contexts"

type="consensus" 
labels_dir="data/biobert/other"

#all files from 00-49
NAME="dis-contexts-w100-[0-9][0-9].tsv"
    ...
    if [[ "${dataset##*./}" =~ ${NAME} ]]; then
        ...
        for model in $MODELS; do
            if [[ "$model" =~ "2595163" ]]; then
                ...
                while true; do
                #change to base for base model
                    jobs=$(ls output-biobert/predictions/base_new_week8_che | wc -l)
                    if [ $jobs -lt $MAX_JOBS ]; then break; fi
                        echo "Too many jobs ($jobs), sleeping ..."
                        sleep 60
                done
            ...
            sleep 5
            if [[ "$model" =~ "2699830" ]]; then touch output-biobert/predictions/base_new_week8_dis/$job_id; fi
            sleep 5
        done
    fi
done
