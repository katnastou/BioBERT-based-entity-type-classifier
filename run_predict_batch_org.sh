...
mkdir -p output-biobert/predictions/organisms
...
data_dir="/scratch/project_2001426/stringdata/week_31_2/species/split-contexts"

type="consensus" 
labels_dir="data/biobert/other"

#all files from 00-49
NAME="org-contexts-w100-[0-9][0-9].tsv"
    ...
    if [[ "${dataset##*./}" =~ ${NAME} ]]; then
        ...
        for model in $MODELS; do
            if [[ "$model" =~ "2595163" ]]; then
                ...
                while true; do
                #change to base for base model
                    jobs=$(ls output-biobert/predictions/organisms | wc -l)
                    if [ $jobs -lt $MAX_JOBS ]; then break; fi
                        echo "Too many jobs ($jobs), sleeping ..."
                        sleep 60
                done
            ...
            sleep 2
            if [[ "$model" =~ "2699830" ]]; then touch output-biobert/predictions/organisms/$job_id; fi
            sleep 2
        done
    fi
done
