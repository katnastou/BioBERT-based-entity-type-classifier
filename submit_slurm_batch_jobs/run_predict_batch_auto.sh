...
mkdir -p output-biobert/predictions/no_blacklist
...
data_dir="/scratch/project_2001426/stringdata/blacklists_paper/tagger-run-2M-blacklist/split-contexts"

type="consensus" 
labels_dir="data/biobert/other"

#all files from 000-199
    ...
    if [[ "${dataset##*./}" =~ ${NAME} ]]; then
        ...
        for model in $MODELS; do
            if [[ "$model" =~ "2595163" ]]; then
                ...
                while true; do
                #change to base for base model
                    jobs=$(ls output-biobert/predictions/no_blacklist | wc -l)
                    if [ $jobs -lt $MAX_JOBS ]; then break; fi
                        echo "Too many jobs ($jobs), sleeping ..."
                        sleep 60
                done
            ...
            sleep 5
            if [[ "$model" =~ "2699830" ]]; then touch output-biobert/predictions/no_blacklist/$job_id; fi
            sleep 5
        done
    fi
done
